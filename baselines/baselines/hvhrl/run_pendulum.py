...
def argsparser():
    ...
    # Task
    parser.add_argument('--task', type=str, choices=['train', 'evaluate', 'sample', 'visualize'], default='train')
    # for evaluatation
    ...
    parser.add_argument('--policy_hidden_size', type=int, default=32)
...
def main(args):
    ...
    if args.task == 'train':
        ...
        policy_run(env, policy_fn, args.checkpoint_dir, number_rollouts=10,
...
def train(env, seed, policy_fn, algo,
          policy_entcoeff, num_timesteps, save_per_iter,
          checkpoint_dir, log_dir, task_name=None):

    if algo == 'trpo':
        from baselines.hvhrl import trpo_mpi
        # Set up for MPI seed
